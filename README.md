...
对于 macOS / Linux 用户，可以直接运行以下命令，脚本会自动下载最新版本并完成 PATH 与 Skill 的配置：

```bash
curl -fsSL https://raw.githubusercontent.com/shijianzhong/multi-platform-articles/main/scripts/install.sh | sh
```

对于 Windows 用户，可以在 PowerShell 中运行：

```powershell
iex ((New-Object System.Net.WebClient).DownloadString("https://raw.githubusercontent.com/shijianzhong/multi-platform-articles/main/scripts/install.ps1"))
```

### 手动安装 (备选)

如果一键脚本执行失败，也可以前往 [GitHub Releases](https://github.com/shijianzhong/multi-platform-articles/releases/latest) 下载对应你系统的压缩包（如 `mpa-v0.1.3-x86_64-apple-darwin.tar.gz`）。
...
mpa
```
在界面中输入你的 `AppID` 和 `Secret`，按 `Ctrl+s` 保存（或在切换字段/分页时自动保存）。配置会安全地存放在你的本地用户目录中。
